...
var (
	...
	patchActor = func(tracker k8stesting.ObjectTracker) func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
		return func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
			pa := action.(k8stesting.PatchActionImpl)
			patchType := pa.GetPatchType()
			if patchType != apitypes.MergePatchType && patchType != apitypes.ApplyPatchType {
				return true, nil, fmt.Errorf("unsupported patch type: %v", patchType)
			}
			return k8stesting.ObjectReaction(tracker)(pa)
		}
	}
)
...
func TestUpdateUnReadyPod(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			fakeKubeClient := fakeclientset.NewClientset(&corev1.Pod{
			...
			fakeKubeClient.PrependReactor("patch", "pods", patchActor(fakeKubeClient.Tracker()))
			...
			if updated {
				waiter.wait()
				updatedPod, err := fakeKubeClient.CoreV1().Pods(podNamespace).Get(context.Background(), podName, metav1.GetOptions{})
...
func TestCheckUnReadyPods(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			annotated, err := checkAnnotation(fakeKubeClient, tc.existingPod)
			require.NoError(t, err)
...
func checkAnnotation(kubeClient *fakeclientset.Clientset, pod *corev1.Pod) (bool, error) {
