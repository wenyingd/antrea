...
func newPodConfigurator(
	...
	// Initiates the PortStatus message listener. This function is a no-op except on Windows.
...
func (pc *podConfigurator) recordPodEvent(ifConfig *interfacestore.InterfaceConfig, installed bool) {
	...
	if err != nil {
		klog.InfoS("Unable to get Pod", "Pod", klog.KRef(ifConfig.PodNamespace, ifConfig.PodName))
		return
	}
