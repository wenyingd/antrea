...
func (m *podIfaceMonitor) updatePodUnreadyAnnotation(podNamespace, podName string, addAnnotation bool) {
	...
	if addAnnotation && !annotated {
		annoPatch := map[string]string{types.PodNotReadyAnnotationKey: ""}
		patch, _ := json.Marshal(annoPatch)
		...
		delete(pod.Annotations, types.PodNotReadyAnnotationKey)
		patch, _ := json.Marshal(pod.Annotations)
		m.kubeClient.CoreV1().Pods(podNamespace).Patch(context.Background(), podName, apitypes.ApplyPatchType, patch, metav1.PatchOptions{})
	}
}
