...
func MoveIFConfigurations(ips []*net.IPNet, routes []interface{}, mac net.HardwareAddr, mtu int, from string, to string) error {
	toInterface, err := getAdapterOrVirtualAdapteByName(to)
	if err != nil {
		return err
	}
...
func GetNetRoutesOnAdapter(linkIndex int) (string, []interface{}, error) {
	...
	for _, r := range rs {
		...
		if r.DestinationSubnet.IP.IsUnspecified() {
			gw = r.GatewayAddress.String()
			continue
		}
...
func getAdapterOrVirtualAdapteByName(ifName string) (*net.Interface, error) {
	var adapter *net.Interface
	var err error
	for _, name := range []string{ifName, fmt.Sprintf("%s (%s)", ContainerVNICPrefix, ifName)} {
		adapter, err = net.InterfaceByName(name)
		if err == nil {
			return adapter, nil
		}
		if opErr, ok := err.(*net.OpError); ok && opErr.Err.Error() == "no such network interface" {
			klog.ErrorS(err, "Failed to find network adapter", "name", ifName)
		} else {
			return nil, err
		}
	}
...
func delUplinkOnSwitch(switchName, ifName string) error {
	cmd := fmt.Sprintf(`Get-VMSwitch -Name "%s"| remove-VMSwitchTeamMember -NetAdapterName "%s"`, switchName, ifName)
	_, err := ps.RunCommand(cmd)
	return err
}
