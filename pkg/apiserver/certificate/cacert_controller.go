...
var (
	antreaListOption = metav1.ListOptions{
...
func (c *CACertController) syncMutatingWebhooks(caCert []byte) error {
	...
	mWebhooks, err := c.client.AdmissionregistrationV1().MutatingWebhookConfigurations().List(context.TODO(), antreaListOption)
	if err != nil {
		return fmt.Errorf("error listing antrea MutatingWebhookConfiguration: %v", err)
	}
	candidateResources := sets.NewString(c.caConfig.MutationWebhooks...)
	optionalResources := sets.NewString(c.caConfig.OptionalMutationWebhooks...)
	for _, mWebhook := range mWebhooks.Items {
		name := mWebhook.Name
...
func (c *CACertController) syncConversionWebhooks(caCert []byte) error {
	...
	cWebhooks, err := c.apiExtensionClient.ApiextensionsV1().CustomResourceDefinitions().List(context.TODO(), antreaListOption)
	if err != nil {
		return fmt.Errorf("error listing antrea CRD definition: %v", err)
	}
	candidateResources := sets.NewString(c.caConfig.CRDsWithConversionWebhooks...)
	for _, crdDef := range cWebhooks.Items {
		name := crdDef.Name
		if !candidateResources.Has(name) {
			continue
		}
...
func (c *CACertController) syncValidatingWebhooks(caCert []byte) error {
	...
	vWebhooks, err := c.client.AdmissionregistrationV1().ValidatingWebhookConfigurations().List(context.TODO(), antreaListOption)
	if err != nil {
		return fmt.Errorf("error listing antrea ValidatingWebhookConfiguration: %v", err)
	}
	candidateResources := sets.NewString(c.caConfig.ValidatingWebhooks...)
	for _, vWebhook := range vWebhooks.Items {
		name := vWebhook.Name
		if !candidateResources.Has(name) {
			continue
		}
...
func (c *CACertController) syncAPIServices(caCert []byte) error {
	...
	antreaAPIServices, err := c.aggregatorClient.ApiregistrationV1().APIServices().List(context.TODO(), antreaListOption)
	if err != nil {
		return fmt.Errorf("error listing antrea APIService: %v", err)
	}
	candidateResources := sets.NewString(c.caConfig.APIServiceNames...)
	for _, apiService := range antreaAPIServices.Items {
