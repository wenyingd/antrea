...
func testExternalFlows(t *testing.T, config *testConfig) {
	...
	if err := c.InstallExternalFlows(nodeIP, *localSubnet); err != nil {
		t.Errorf("Failed to install OpenFlow entries for Pod accessing the external addresses: %v", err)
	}
	...
	return []expectTableFlows{
		{
			...
			[]*ofTestUtils.ExpectFlow{
				{
					fmt.Sprintf("priority=200,ip,in_port=%d", types.UplinkOFPort),
					"load:0x3->NXM_NX_REG0[0..15],resubmit(,30)",
				},
			},
		},
		{
		...
		},
		{
			...
			[]*ofTestUtils.ExpectFlow{
				{
					"priority=210,ct_state=-new+trk,ct_mark=0x40,ip,reg0=0x3/0xffff",
