...
var (
	...
	portFoundMark   = binding.NewRegMark(0, 1, 16, 16)
	portCacheField  = binding.NewRegField(1, 0, 31)
	fromLocalMark   = binding.NewRegMark(0, 2, 0, 15)
	fromGatewayMark = binding.NewRegMark(0, 1, 0, 15)
...
func prepareFlows(table binding.Table) ([]binding.Flow, []*ExpectFlow) {
	...
	mark0 := binding.NewRegMarkWithField(regField0, 0x0fff)
	regField1 := binding.NewRegField(0, 16, 31)
	mark1 := binding.NewRegMarkWithField(regField1, 0x0ffe)
...
func prepareNATflows(table binding.Table) ([]binding.Flow, []*ExpectFlow) {
	...
	snatMark1 := binding.NewRegMark(marksReg, 1, 17, 17)
	snatMark2 := binding.NewRegMark(marksReg, 1, 18, 18)
	dnatMark1 := binding.NewRegMark(marksReg, 1, 19, 19)
	dnatMark2 := binding.NewRegMark(marksReg, 1, 20, 20)
	flows := []binding.Flow{
		table.BuildFlow(priorityNormal).MatchProtocol(binding.ProtocolIP).
